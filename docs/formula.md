...
- Структурные микросостояния: `cyclomatic`, `nesting`, `length`
  (v0.1) + `coupling` (v0.3 MVP — per-file import count, см. §«Микросостояния»).
...
```
S_file = статическая структурная энтропия (cyclomatic + nesting + length + coupling, далее duplication)
